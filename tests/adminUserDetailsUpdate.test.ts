import { adminUserDetailsUpdate, adminAuthRegister } from '../src/auth.ts'
...
describe('Testing adminUserDetailsUpdate for errors', () => {
  ...
  test('Email is already used by another user', () => {
    const authUserId2 = adminAuthRegister('steph@unsw.edu.au', 'Farmingsimulator!1234', 'steph', 'liang').authUserId;
    const result = adminUserDetailsUpdate (authUserId2, 'amelia@unsw.edu.au', 'steph', 'liang');
    expect(result).toStrictEqual({ error: 'email used by another user' });
  });

  // Email is not valid
  test('Email is not a valid email', () => {
    const result = adminUserDetailsUpdate(id, 'gurigiurabgiurag', 'amelia', 'su');
  ...
  test('First name contains invalid characters', () => {
    const result = adminUserDetailsUpdate(id, 'amelia@unsw.ed.au', 'a!melia', 'su');
    expect(result).toStrictEqual({ error: 'first name contains invalid characters' })
  });

  test('Last name contains invalid characters', () => {
    const result = adminUserDetailsUpdate(id, 'amelia@unsw.ed.au', 'amelia', 'su+');
  ...
  test('First name is too short', () => {
    const result = adminUserDetailsUpdate(id, 'amelia@unsw.ed.au', 'a', 'su');
    expect(result).toStrictEqual({ error: 'first name is too short' });
  });

  test('First name is too long', () => {
    const result = adminUserDetailsUpdate(id, 'amelia@unsw.ed.au', 'abcdefghijklmnopqrstuv', 'su');
    expect(result).toStrictEqual({ error: 'first name is too long'});
  });

  test('Last name is too short', () => {
    const result = adminUserDetailsUpdate(id, 'amelia@unsw.ed.au', 'amelia', 's');
    expect(result).toStrictEqual({ error: 'last name is too short' });
  });

  test('Last name is too long', () => {
    const result = adminUserDetailsUpdate(id, 'amelia@unw.ed.au', 'amelia', 'abcdefghijklmnopqrstuv');
